...
public class  GameOverPanelAnimation : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        LeanTween.scale(yourScoreRectTransform, Vector3.one, 0.2f).setEaseInOutBack().setIgnoreTimeScale(true);
        LeanTween.scale(totalScoreRectTransform, Vector3.one, 0.2f).setEaseInOutBack().setDelay(0.3f).setIgnoreTimeScale(true);
        LeanTween.move(panelRectTransform,new Vector3(0,-100,0),0.2f).setEaseOutCubic().setDelay(0.6f).setIgnoreTimeScale(true);
    }
}
